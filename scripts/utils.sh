...
if [[ "$COMMAND" == "docker-build" ]]; then
    bash -c "source ./scripts/pg_docker_build.sh -f ./docker/dev.Dockerfile --build-arg RUN_SMOKE_TEST=true -t $2"
    ...
    bash -c "source ./scripts/pg_docker_build.sh -f ./docker/base.Dockerfile -t ghcr.io/covid-genomics/bioseq-postgres-dev:latest"
