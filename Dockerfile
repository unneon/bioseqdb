...
RUN apk add --no-cache autoconf automake bzip2-dev cmake g++ git make openssl postgresql-dev xz-dev zlib-dev \
    ...
    && git clone --depth 1 --single-branch "https://github.com/lh3/bwa.git" /usr/src/bwa \
    && cd /usr/src/htslib && autoreconf -i && ./configure --host x86_64-pc-linux-musl && make CFLAGS="-g -O2 -fPIC" lib-static \
